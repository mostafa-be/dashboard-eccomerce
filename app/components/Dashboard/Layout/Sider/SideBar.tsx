...
import { ChevronDown, ChevronLeft, ChevronRight, Dot } from "lucide-react";
...
export const SideBar = ({ expanded, setExpanded, children }: PropsSidebar) => {
  ...
  return (
    <aside
      ...
      } transition-all transform ease-in-out duration-300 origin-${
        isRTL ? "right" : "left"
      } sticky top-0 ${isRTL ? "right-0" : "left-0"}`}
    >
      <div className="w-full bg-transparent">
        <div className="w-full flex items-center justify-center my-5">
          ...
          <button
            ...
            } z-50 overflow-hidden w-10 h-10 bg-blue-650 p-1.5 rounded-full flex items-center justify-center`}
  ...
  return (
    <>
      {links ? (
          ...
          >
            ...
            {alert && (
              <div
                ...
                } w-2 h-2 rounded bg-red-400 ${expanded ? "" : "top-2"}`}
              />
            )}
            {!expanded && (
              <div
                className={`hidden group-hover:flex flex-col absolute min-w-24 ${
                  isRTL ? "right-20" : "left-20"
                } top-1/2 -translate-y-1/2 bg-blue-650 text-white px-4 py-5 gap-y-1.5 rounded-lg shadow`}
              >
                {groupLinks &&
                  groupLinks.map(
            ...
            {!expanded && (
              <div
                className={`hidden group-hover:block absolute ${
                  isRTL
                    ? "right-[74px] border-l-[7px] border-l-blue-650 border-r-0"
                    : "left-[74px] border-r-[7px] border-r-blue-650 border-l-0"
                } top-1/2 -translate-y-1/2 w-0 h-0 
        ...
        >
          <Link
            className="w-full flex items-center gap-3 py-3 px-3"
            href={`${url}`}
          >
            {icon}
            <span
              className={`overflow-hidden ${
                expanded ? "w-48 " : " hidden"
              }`}
            >
              {text}
            </span>
            ...
            {!expanded && (
              <div
                className={`hidden w-max group-hover:flex flex-col absolute ${
                  isRTL ? "right-20" : "left-20"
                } bg-blue-650 text-white p-3 rounded-lg shadow`}
              >
                {text}
              </div>
            )}
